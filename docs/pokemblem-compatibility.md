...
exact sampled-pixel match. The extended unit renderer found four standard map
sprite handles and rendered an actor below the native GBA viewport, confirming
that the existing actor abstraction also applies to the free-roaming map.
...
mGBA frame, menus and overlays, and extended actors.

## Remaining edge cases

The canonical mGBA frame remains authoritative for moving-object animations,
dialogue, menus, and effects. Standing actors outside that frame are decoded
from FE8 map-sprite state. Any Pokémblem scene that replaces those structures
will safely fall back, but needs its own validated actor provider before its
off-screen moving objects can be extended.
